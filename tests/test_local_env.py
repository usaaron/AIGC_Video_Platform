...
def test_missing_local_env_without_exported_overrides_is_valid(tmp_path):
    result = subprocess.run(
        [shutil.which("bash") or "bash", "-euc", 'source scripts/load_local_env.sh\nload_local_env "$1"',
         "test", (tmp_path / "missing.env").as_posix()],
        env={"PATH": os.environ["PATH"], **{key: os.environ[key] for key in ("SystemRoot", "SYSTEMROOT", "WINDIR") if key in os.environ}}, text=True, capture_output=True,
    )
