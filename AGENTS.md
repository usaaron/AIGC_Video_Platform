...
- 剧本页“小说上传与章节”正式用户流程；长剧本创作由功能栈中的独立 Script Master 服务承载。
- 音频资产 AI 生成、配音、字幕、混音和正式交付导出。
- 混元图片模型 Provider。
- 正式商用支付、发票、税务和用户数据导出/删除。

功能栈入口不再统一归类为占位：一句成片暂未开放，图片大师已接入，独立剧本大师已部署。剧本大师的目标项目绑定、派生修订版和交接失败恢复仍有缺口，见 `docs/SCRIPT_MASTER_ADAPTATION_REVIEW_2026-09-15.md`；不得将入口可用等同于完整交付闭环。
