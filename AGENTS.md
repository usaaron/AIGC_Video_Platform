...
1. `docs/PROJECT_HANDOFF.md`：面向同事的全量架构、流程、部署和运维交接。
2. `docs/CURRENT_STATE.md`：当前真正开放的功能、占位功能、生产状态和已知风险。
3. `docs/HANDOFF_GUIDE.md`：30 分钟项目总览和代码地图。
4. `docs/README.md`：按任务选择专项文档。
...
“一句成片”AgentStudio 与“生图大师”已接后端，实际 Provider 生成质量仍需验收。
...
- Seedance 单镜头请求启用音频；FFmpeg 完整预览保留源音轨，统一采样率和声道，无音轨镜头补静音。正式配音、混音和字幕仍未交付。
- 图片 Provider 当前只有 TokenAdvent GPT Image 2；混元图片模型选择项处于未配置禁用状态。
