...
- 真实模式会读取 `apps/api/.env` 中的 `TEXT_MODEL` 并按模型选择对应 Provider；默认 `TEXT_MODEL=glm-5.2`。GLM/Kimi 模型需要 `REHDASU_API_KEY`、`REHDASU_BASE_URL` 和 `REHDASU_CHAT_COMPLETIONS_PATH`；SEQORA 5.4/5.5/5.6 需要 `TOKENADVENT_API_KEY`；DeepSeek V3 需要 `DEEPSEEK_API_KEY`（或复用 `STRINGX_API_KEY`）、`DEEPSEEK_BASE_URL` 和 `DEEPSEEK_MODEL`。
