...
> 核对日期：2026-08-21。本文描述当前产品状态。若与历史开发记忆冲突，以 contracts、Service、Repository、实际 UI 和本文为准。
...
| 账号中心       | 已开放                 | Postgres + 本地兼容         | 身份与邮箱验证、实时积分和月度用量、当前账号数据、设备 session；多组织用户才显示空间切换      |
...
| 功能栈入口     | 部分开放               | 生图大师已接工作流          | 对话一句成片、剧本大师仍是预览/待开发；生图大师已是正式图片工作台                             |
...
| 资产库         | 已开放                 | 账号级库 + ObjectStorage    | 可保存、列表、下载、包下载、版本、去重、回收站和导入当前项目；trustedAssets 单独保留          |
| 生图大师       | 已开放                 | 服务端托管 img2/image2      | 专用批次接口、服务端扣费、引用图角色、提示词优化、视觉解析、快照、重试和结果复用              |
...
- 剧本默认模型是 `glm-5.2`。当前 UI 可选择序幕TV 5.6、序幕TV 5.6 Terra、Kimi K3、GLM 5.2、GLM 5.2 Fast；内部 `seqora-5.6` 是服务端实际 GPT 5.6 模型的对外别名，只有配置了对应 Provider 的模型才能真实调用。
...
- 网剧提示词按每场 4 到 15 秒控制目标时长，要求场内动作微节拍、表情/配角反应、对白/画外音/内心独白和结尾钩子。分镜仍要人工抽检，不应宣称模型必然满足。

### 资产

- 图片来源分为直接使用原图、参考图再生成和纯提示词生成。正式图片工作台对外叫“生图大师”，旧“图片大师/序幕 image2”只作为历史说法。
- 资产库是账号级长期资产集合，项目资产设计仍按项目隔离；项目资产和生成结果可以入库，也可以从资产库导入当前项目。
...
- 后端继续保留 `organizationType: personal` 和个人账号隔离结构，但普通用户界面不强调“组织：个人”或“个人空间”；单一普通账号只显示“当前账号数据”。只有用户加入多个企业/团队时，才显示“切换组织/空间”。
- 管理后台可以保留“个人账号”用于运营管理 C 端用户的积分、状态、登录和资产；企业/团队才叫“组织空间”。`organization_admin` 是组织负责人，可通过邮件邀请 `organization_member`，但不能创建或提权其他组织负责人。平台 `owner` 与组织负责人是两层不同权限。
- 无 Docker 的本地 JSON 模式保留兼容账户接口，方便创作端联调；真实多组织、成员邀请、跨设备 session 撤销只在 Postgres 运行时可用，不能把本地回退误认为生产组织能力。
...
1. 通用 `generation_tasks.estimatedCredits` 仍可由客户端提交，需要服务端定价表和报价确认，避免伪造低价格；生图大师专用接口已改为服务端定价。
...
`image-studio` 已切换为正式的“生图大师”工作台。前端只负责提示词、引用图、画幅/尺寸、生成质量和张数，批次提交走服务端专用接口，浏览器不再保存 provider key，也不再直接信任估算积分。详见 `docs/IMAGE2_STUDIO.md`。

## 记忆索引

新会话先按 `CONTEXT-MAP.md` 进入对应上下文。近期容易混淆的三条事实：生图大师不是旧占位，资产库不是项目资产，个人账号不是普通用户界面里的“个人组织”。
