...
2026-09-21 视频修复：Dora 查询异常继续跟踪原任务，超时退款后进入最长 24 小时的后台结果核对；补齐生成图依赖，避免人物/场景参考图在 Worker 缓存中丢失。AI 人物入库与视频上游独立审核分别展示。实现与验证边界见 [视频自动接回与人物参考图修复](VIDEO_AUTO_RECONCILIATION_2026-09-21.md)。
...
- 2026-09-21 视频等待修复：DoraRouter 未返回真实进度时的 5%/50% 仅作阶段显示，不能作为停滞证据；不支持远端取消的 Provider 不再因短时停滞自动重新提交。视频单次远端等待上限默认 30 分钟（`VIDEO_PROCESSING_TIMEOUT_MS`），超过上限保留远端任务编号、失败退款，不自动重提；上游返回完成时先接回结果。已超时但上游成功的历史任务需经过审计恢复结果，不能据失败文案判定上游没有成品。
