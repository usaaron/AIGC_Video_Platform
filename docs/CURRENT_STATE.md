...
- 剧本默认模型是 `deepseek-v4-flash`。模型下拉框依次展示 DeepSeek V4 Flash、DeepSeek V4 Pro、GLM 5.2、序幕-5.6；当前 GLM Key 未获得模型授权，因此 UI 保留位置但禁用选择。序幕-5.6 实际路由到 `gpt-5.6-sol`，旧 `seqora-5.6` 任务也映射到 Sol。
