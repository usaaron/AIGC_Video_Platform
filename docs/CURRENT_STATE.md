...
> 2026-09-21 本地候选，尚未部署：补齐快速剧本到主站资产、分镜的资料复用。v2 正文交付携带与正文哈希绑定的有序场次依据；资产页自动读取人物、场景和物品，仍须「确认加入资产」后创建。分镜使用经校验的逐场出场清单，沉默角色也可关联；无图记录显示「待设计」，不会占图片参考名额。仅资料变化不覆盖已有分镜；旧稿或失效依据回退正文解析。检查与边界见 [剧本资产与分镜衔接](SCRIPT_ASSET_STORYBOARD_HANDOFF_2026-09-21.md)。下方已发布记录仍代表生产状态。

> 2026-09-21 04:07:07（上海时间）快速剧本恢复与助手布局已发布：修复创作安排在 120 秒被本地截止时间中断及原因丢失，快速请求采用独立的 480 秒预算，失败只重试当前步骤。无内容时助手占主要空间；海外两集入口、逐句中英台词及主站交付衔接补齐。剧本 API / Web 为 `e2d4aa71dd17`，主站、两个数据库及 Redis 容器保持不变。前端 1432 项、后端相关 281 项、真实独立 PostgreSQL 4 项及生产合成样本验收通过；未调用付费模型，详见 [恢复与海外两集发布](QUICK_DEMO_RECOVERY_2026-09-21.md)。

> 2026-09-21 03:13:32（上海时间）旧项目两集入口修复已发布：原始资料填写 1–7 集时，可明确进入对应短集数的快速创作，保存资料后保留集数及每集约 1000 字目标，不会卡在标准流程的至少 8 集限制。线上已实测 2 集按钮和跳转。仅剧本 Web 更新为 `dbb3386baced`，剧本 API 仍为 `53364bb946e2`，主站/数据库实例均未变；未调用付费模型。详见 [两集入口发布记录](https://github.com/usaaron/AIGC_Video_Platform/blob/codex/script-master-quick-preview-20260920/deploy/SHORT_SERIES_ENTRY_2026-09-21.md#生产发布)。

> 2026-09-21 02:16:57（上海时间）剧本交互与恢复修复已发布：新建进入原始资料，快速篇幅随显式集数修改自动安排；人物/分集实时过程、切页恢复、长分集目录和紧凑人物标签已补齐。剧本 API / Web 为 `53364bb946e2`，主站与数据库实例保持原版。前端 1409 项、后端 246 项、真实 PostgreSQL 3 项及生产衔接冒烟通过；未调用付费模型，详见 [本次发布记录](SCRIPT_DEMO_RESILIENCE_2026-09-21.md)。下方记录保留为历史状态。

> 2026-09-21 01:01:02（上海时间）剧本线上修复已发布：新建网剧默认时长从 60 秒对齐为 90 秒，生产浏览器实建项目已进入三步快速页并保存；旧项目可显式进入快速创作，已有梗概保留。同时修复 PostgreSQL 驱动 `rowcount=-1` 导致生成检查点误报保存冲突的问题。主站 Web 为 `2cc67d2b591b`，剧本 API / Web 为 `e71ee9874823`；主站 API / Worker 保留 `ebb55fa3c7a9`，两个 PostgreSQL 与 Redis 实例未变。真实 PostgreSQL 回归 3 项和生产样本冒烟通过，未执行付费生成或模型质量验收。最新镜像、备份及验证边界见 [剧本修复发布记录](QUICK_SCRIPT_ENTRY_FIX_2026-09-21.md#生产修复发布)。

> 2026-09-21 00:29 快速剧本融合版已发布：用户确认的本地 5182 版本现运行于 https://xumutv.com。主站业务提交 `ebb55fa`，配套剧本服务 `5f41a27`；模型配置同步为新版 Flash，生产登录、快速状态、交付、资产、规则分镜与制作修订冒烟通过。本轮未调用付费生成；详细镜像、备份和验证边界见 [生产发布记录](QUICK_SCRIPT_DEPLOYMENT_2026-09-21.md)。以下“本地未发布”说明保留为历史记录。
