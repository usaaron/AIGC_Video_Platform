...
模板插入后，本次编辑中的引用正文以深绿色文字和浅绿色底纹标识，输入和删改时调整标识位置；人物、场景与物品的引用仍保留原有背景标记。颜色是编辑器草稿状态，保存及生成只提交纯文本，不写入颜色标签；关闭编辑器后不保留本次颜色范围。

## 人物版本与镜头引用

人物资产共用一份基础身份和已确认面部，各套稳定服饰保存为 appearanceVariants，显示名称统一为“人物名-标准版本 / 人物名-具体服装版本”。旧名称“xx版”与“xx版本”按同一造型去重，保留已有图片、版本 ID 和当前选择；不批量改写已有剧本。

剧本生成、重写、补全和续写要求人物清单填写基础人物、版本、稳定外形及服装；续写上下文包含已保存版本的名称与描述。明确换装才增加版本，同一套服饰跨集复用，临时情绪或动作不另建版本。全剧扫描按版本合并，兼容同一人物名配不同版本字段的旧剧本；正文中的人物本名不会额外生成一套空白标准造型。

资产设计显示当前版本和可用版本，在生成人物前即可修改版本名称与服饰描述，重复名称会提示复用。新增换装沿用面部基准；旧人物已有身体图时，先保留为标准版本。批量生成仍跳过已有图片及排队中的造型。

分镜编辑上方可直接插入人物的具体版本名；选择参考图时优先匹配镜头写明的版本，未指定则使用当前版本。指定版本尚未生成时只沿用面部，不借用另一套衣服的图片。视频保留加白人像引用，同时携带所选版本的身体图与服装描述。相关编译规则版本为 seedance-storyboard-v15；Provider 的实际出图效果仍取决于模型，验证使用测试替身，不产生付费生成。

画面提示词编辑器桌面宽度增至 1240px，输入区高度随窗口在 420–600px 之间调整，手机端至少 360px；保留模板绿色高亮与弹窗滚动。
