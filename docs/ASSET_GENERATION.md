...
人物编辑器按“阶段导航 -> 当前阶段参数 -> 生成结果”的顺序排版。面部阶段的真人/动物、性别、年龄和画风选项位于“身份锚点”结果区上方；全身阶段的体型和背景使用同一位置。人物资产卡优先使用当前造型或已确认的全身参考，再回退到资产输出、面部基准和可信人像预览；面部编辑阶段仍独立展示面部基准。后续阶段同时检查确认状态和实际图片引用，旧数据仅有 `approved` 标记但缺失图片时不能继续生成。

用户明确激活的历史造型，只要它的全身图与当前已确认基准一致，就允许继续生成三视图；不会因存在较新的候选任务而误锁。缺失图片引用或没有确认的新候选仍不能直接继续。
...
可信人像卡片与放大预览统一使用受鉴权保护的 `/api/v1/trusted-assets/portraits/:assetId/preview`，不把 `asset://` 或历史上游链接直接交给浏览器。Dora 图片下载只读取素材详情和图片；普通用户预览尚未绑定的真人素材时，服务端仍核验组类型，素材组详情无权限可通过服务端分组列表确认，未验证的组或他人的 AIGC 资源继续拒绝访问。上游相对图片地址按 Provider 地址解析，下载图片时不转发 Provider Key。图片加载失败时卡片保留重试入口。
...
Worker 的活跃任务缓存将 `references` 和 `images` 中引用的已完成图片任务纳入依赖闭包，因此服务重启或缓存精简后，人物全身、三视图和分镜图片仍能读取已确认的生成图。媒体引用统一解析相对地址、完整 HTTP(S) 地址和查询参数，并继续校验项目、组织及图片类型；其他组织或未完成任务的输出不能作为本地图片引用。
...
`POST /api/v1/projects/:projectId/shots/generate` 不调用文本 Provider，提供两种确定性规则模式。默认 `scene` 按已有导演规格、场次时长和有序叙事事件规划，只有没有时长的旧格式保留一场一镜兼容。高级 `beat` 拆明确动作编号、分号或完整句动作，逗号不作为切镜点；不再只取前四个动作，剩余对白交给末镜。请求镜数上限为 120，完整规划超过请求上限时返回 `SHOT_LIMIT_EXCEEDED` 并保留原分镜，不静默截断。两种模式都不预生成通用演示图片；完成视频直接在分镜卡片中预览。
...
该 MP4 是客户检查镜头顺序和节奏的有声预览，不再次调用 Seedance，也不扣积分。FFmpeg 保留源音轨并统一为 48 kHz 双声道，补齐或裁剪到镜头时长，无音轨镜头补静音；正式配音、混音、字幕和交付导出尚未接入。音轨保留是原分支已有行为。
