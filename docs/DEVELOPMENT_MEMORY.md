...
## 2026-09-18 11:47 创作端 UI 与页内网剧创作发布

主站 Web `daa00829448f` 与剧本大师 Web `3bace156a3b5` 已上线。网剧从主站剧本页创作，保留主站导航与切页草稿；短片和广告沿用原生成方式。发布只重建两个 Web 容器，后端、Worker、数据库及模型密钥保持原状。876 项前端单测、51 项浏览器用例有通过结果；全仓库检查在本机 Docker 数据库预检处阻断，不得称数据库全量测试通过。线上账号登录、绑定项目启动、签名读取、嵌入策略、权限和健康检查通过，未运行付费生成。详见 [发布记录](STUDIO_INLINE_RELEASE_2026-09-18.md)。
...
独立剧本大师源码仍在相邻仓库，未随主项目包部署；服务器未配置剧本大师启动地址和共享密钥。支付为模拟界面，加白接口 `404` 未修复，未运行付费生成验收。备份与发布日志位置见 [发布记录](OPERATIONS_RUNBOOK.md#2026-09-15-合并分支主项目发布)。本条部署发生在下方配置更新之后，当前运行镜像以本条为准。
...
- 中文短素材按内容类型生成：广告/短片仍按目标秒数组织；网剧单集默认一次只生成 1 集、6～8 场、约 700～1100 个中文字符，导演级构图、光影和运镜留给分镜层，以缩短模型响应。已有多场结构化素材即使少于 1500 字也按原场次数量改写，不进入自由扩写；大量英文输出会自动校正一次，仍异常则禁止覆盖。
- 人物本地导入或 AI 生成。
