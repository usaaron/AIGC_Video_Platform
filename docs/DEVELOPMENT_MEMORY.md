...
> 当前目标：稳定封闭客户测试，补齐剩余服务端定价、带音轨完整成片、邮件运维告警和可恢复 Agent 编排。
...
## 2026-08-21 当前快照（覆盖下方历史表述）

- 根目录新增 `CONTEXT-MAP.md`，按 Creative Platform 和 Admin Console 两个上下文索引当前词汇；新会话应先用它校准“生图大师”“资产库”“个人账号”“组织空间”等概念。
- `image-studio` 对外统一叫“生图大师”。浏览器不保存 Provider Key，不显示 API 地址，不直接信任 estimatedCredits；正式提交走 `POST /api/v1/image2/batches`，服务端托管 Provider、模型、积分和任务创建。
- 生图大师支持最多 5 张输入图：主体图最多 1 张，非主体引用图最多 4 张；角色包括主体、服装、帽子/配饰、风格、构图、色调。提示词优化和引用图视觉解析由后端托管 chat，默认辅助模型 `gpt-5.4`。
- 生成快照必须保留最终提示词、负面提示词、引用图顺序/角色、清晰度、画幅/尺寸和辅助能力状态。严格重做复用快照，不再次优化提示词；编辑恢复最终提示词并按当前规则清空其他引用图。
- 资产库是账号级长期资产集合，项目资产设计仍按项目组织。`trustedAssets` 继续单独保留，media 只负责底层文件。资产库支持保存、列表、下载、包下载、版本、去重、回收站和导入当前项目。
- 个人账号语义已校准：后端继续保留 `organizationType: personal`，但普通用户界面不强调“组织：个人”或“个人空间”；管理后台可保留“个人账号”，企业/团队才叫“组织空间”。
- 管理后台合规规则已抽到独立配置，风险标签展示命中的词和命中原因，并支持白名单语境和项目级阈值。当前先用可解释规则引擎，模型二审留作后续。
- 测试入口已稳定化：`pnpm test:full:stable` 是稳定全量验收，API DB 测试串行使用共享测试 DB + schema 隔离；`pnpm test:e2e` 是独立 Playwright mock API 套件，不调用真实 Provider、不扣积分、不依赖生产 DB。

## 2026-08-03 当前快照（历史基线，若与 2026-08-21 冲突以上方为准）
...
- 中文短素材默认由一次后台模型调用生成适合 15 到 30 秒视频的 4 到 6 场、约 1800 到 2600 字制作稿，并同时补齐动作、对白、构图、光影、运镜和衔接。已有多场结构化素材即使少于 1500 字也按原场次数量改写，不进入自由扩写；大量英文输出会自动校正一次，仍异常则禁止覆盖。
- 人物本地导入或 AI 生成。
...
- 本地对象存储和 GCS 对象存储接口。
- 分镜提供“按场次智能生成”和“按动作拆分镜头”：默认场次模式一场对应一个 4 到 15 秒视频镜头，场内动作是同镜微节拍；按动作拆分镜头只拆明确动作编号、分号或完整句，不按逗号制造镜头、不合成假动作，也不把同一段对白重复到每一镜。两种模式都不调用文本 Provider，另有手动添加镜头。
...
- 批量视频提供“分段并发 / 全片串联 / 全部独立”。默认按剧集建立连续链，每集第一镜独立，后续镜头承接真实尾帧；分段并发只并发独立链，全部独立由用户确认后主动放弃连续性。
...
分镜保存顺序、标题、景别、时长、提示词、负面提示词、连续模式和 `imageUrl`。默认场次模式按非空场次一场一镜；按动作拆分镜头只处理明确动作节拍，接口上限 120 镜。两者都不写入旧版通用演示图片。分镜卡片会直接播放当前选中的已完成视频。当前接口仍使用 `replaceShots()` 直接删除项目旧分镜并创建新 ID，没有二次确认或撤销，旧生成任务仍引用旧镜头 ID。
...
- 分镜页的“按场次智能生成”“按动作拆分镜头”和“手动添加”是三个明确入口；前两者都不调用文本 Provider。默认一场一镜，只有场内确实包含多个无法在同镜完成的明确动作时才由用户主动使用按动作拆分镜头。
...
- 分镜契约包含 `continuityNote`，旧项目读取时默认空字符串。备注优先记录结构化入场/出场状态和“上一镜已完成、不得重演”的边界，进入 Seedance 视频提示词并可手工修改；不要把上一整场原文重复发送给每个镜头。真实 `last-frame -> first_frame` 始终是连续模式第一优先级，文本只补充人物位置、动作方向、服装、物品和光线状态。
