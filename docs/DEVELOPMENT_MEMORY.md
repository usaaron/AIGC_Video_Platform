...
- 本地对象存储和 GCS 对象存储接口。
- 分镜提供“按场次智能生成”和“高级：动作级细拆”：默认场次模式一场对应一个 4 到 15 秒视频镜头，场内动作是同镜微节拍；高级模式只拆明确动作编号、分号或完整句，不按逗号制造镜头、不合成假动作，也不把同一段对白重复到每一镜。两种模式都不调用文本 Provider，另有手动添加镜头。
...
分镜保存顺序、标题、景别、时长、提示词、负面提示词、连续模式和 `imageUrl`。默认场次模式按非空场次一场一镜；动作级细拆只处理明确动作节拍，接口上限 120 镜。两者都不写入旧版通用演示图片。分镜卡片会直接播放当前选中的已完成视频。当前接口仍使用 `replaceShots()` 直接删除项目旧分镜并创建新 ID，没有二次确认或撤销，旧生成任务仍引用旧镜头 ID。
...
- 分镜页的“按场次智能生成”“高级：动作级细拆”和“手动添加”是三个明确入口；前两者都不调用文本 Provider。默认一场一镜，只有场内确实包含多个无法在同镜完成的明确动作时才由用户主动使用高级细拆。
