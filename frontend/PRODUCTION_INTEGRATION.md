...
Unresolved standalone delivery: an unscoped Script Master project still falls back to its own ID as the host delivery target. If that ID has no corresponding host project, the host will reject delivery. Choosing or creating the host destination for standalone projects remains a backend/host product decision; this frontend change adds no destination-picker workflow. Scoped delivery uses bearer authentication without host cookies so a concurrent cookie-account switch cannot change the delivery identity.
