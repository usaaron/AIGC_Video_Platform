...
function fixture(t, fetchResponse) {
  ...
  const firstRequestBoundary = deferred();
  ...
  const start = (beforeStart) => enqueuePlanningTask({
    ...
    run: async () => {
      ...
      const result = await decomposeStoryPlanNode(project, parent, undefined, {
        beforeRequest: async () => {
          const resumed = waitForPlanningTaskResume(key);
          firstRequestBoundary.resolve();
          await resumed;
        },
      });
      saved.push(...result);
  ...
  return { project, children, calls, timers, saved, start, firstRequestBoundary, firstFetchStarted,
...
test("pause before the first decomposition POST waits without issuing a request", async (t) => {
  ...
  await tick(); f.pause(); ready.resolve();
  // Operation identity hashing is async; wait for the real request boundary,
  // not an assumed single event-loop turn, before checking the pause state.
  await f.firstRequestBoundary.promise;
...
test("503 after pause completes recovery reads but cannot POST again until resumed", async (t) => {
  ...
  const queued = f.start(); await f.firstFetchStarted.promise; await tick();
...
test("a successful in-flight response is checkpointed while paused without replay", async (t) => {
  ...
  const queued = f.start(); await f.firstFetchStarted.promise; f.pause();
...
test("a complete server checkpoint recovered after 503 is saved while paused", async (t) => {
  ...
  const queued = f.start(); await f.firstFetchStarted.promise; f.pause(); response.resolve(unavailable());
  assert.deepEqual(await queued.promise, f.children);
  assert.deepEqual(f.saved, f.children);
  assert.deepEqual(f.calls.map((call) => call.method), ["POST", "GET"]);
