...
> 最新生产修复：2026-09-21 01:01:02（上海时间）已发布业务提交 `e71ee9874823`，修复 PostgreSQL 生成检查点保存误报冲突，并配套主站新网剧默认进入快速版的入口修复。真实 PostgreSQL 回归 3 项和生产样本冒烟通过；未执行付费生成，不代表真实模型质量已验收。镜像、备份和验证边界见 [发布记录](deploy/POSTGRES_GENERATION_SAVE_FIX_2026-09-21.md#生产发布与验证)。以下长篇流程说明同时适用于保留的独立工作台和旧项目。
