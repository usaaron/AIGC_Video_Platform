...
- `textProvider`: defaults to DeepSeek V4 Flash for Chinese understanding, scripts, summaries, and asset suggestions. DeepSeek V4 Flash/Pro use `DEEPSEEK_V4_API_KEY`, GPT uses `TOKENADVENT_API_KEY`, and GLM/Kimi use `REHDASU_API_KEY`; a model is only usable when its route and model authorization are both configured.
