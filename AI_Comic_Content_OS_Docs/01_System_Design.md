...
- 当前已加入基础阶段生成边界：按项目总集数分有界批次生成，批次间允许更新创作输入；PostgreSQL/JSONB schema、migration 和 Repository foundation 已实现，但 API 切换、后台任务恢复与专业长篇规划仍待实现
