...
当前产品核心是优质长剧本生成，目标支持约 60 万字正文规模。所有近期能力必须能够说明它如何提升长篇的整体结构、连续性、正文质量、可恢复生成或可验证性；单集效果、界面功能或媒体生产便利性不能反向挤占该核心目标。
...
“禁止无治理自由标签”不等于禁止用户创作新概念。未知标签可以先成为 project-scoped、带用户定义和 inference provenance 的临时上下文；只有经过 alias 去重、来源验证和治理审核后，才可提升为公共 Ontology / Knowledge 候选。实时热度必须与稳定标签定义分离。
...
长篇结构采用 level-free、允许分支不同深度的递归拆分。规划树不为视觉整齐而强制固定卷/章/单元层级；每个节点根据叙事复杂度独立达到 `episode_ready`。

人物关系网和故事线视图只投影同一组版本化事实。修改默认只影响未来内容，历史改写必须显式评估影响范围、创建版本并保留审计链。

人物、关系和故事线可以随确认内容渐进形成，但未确认生成候选不得成为连续性事实。关系与故事线保持独立语义，通过引用形成统一故事地图；生成上下文必须按当前规划范围裁剪，不得因长篇增长而无限注入全量历史。
...
本团队的产品范围止于正式媒体生成之前：由系统从已确认剧本派生文字分镜、资产设计、图片／视频提示词与版本化前期制作包，下游团队执行图片、语音、视频生成与剪辑。前期产物保留正文来源，摄影和外观设计建议不能静默写回叙事事实；用户通过结果、局部修改、锁定和版本选择掌握控制权，不要求操作无限画布。具体待实施方案见 [44_PREPRODUCTION_STORYBOARD_PLAN.md](44_PREPRODUCTION_STORYBOARD_PLAN.md)。
