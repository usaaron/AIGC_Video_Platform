...
2026-09-08 范围补充：用户已将正文之后的分镜、文字资产设计、图片／视频提示词和前期交付包纳入项目方案，详见 [44_PREPRODUCTION_STORYBOARD_PLAN.md](44_PREPRODUCTION_STORYBOARD_PLAN.md)。该扩展尚未实现；实际媒体生成由下游团队承担。本节历史规模与市场说明的当前运行口径仍以 Current Status 为准。

最近交互收口（2026-09-04）：规划确认只负责锁定规划并解锁正文工作区，不再把确认跳转转换为 `?generate=1` 首次生成意图；正文首次生成由工作区中的明确按钮触发。该规则不影响已有正文的受控续写和中断恢复。
...
## Preproduction Delivery Plan

方案已纳入，运行能力待实现。按 D-027，前期制作与实际媒体生成分开：本团队产出文字分镜、资产设计及图片／视频生成描述，用户审阅确认后交给下游。分镜不要求整部长篇完成才开始，也不增加无限画布。

1. 第一期：复用已有正文与场景清单，改进动作、空间和状态输入要求；建立源快照、独立分镜状态、左侧“分镜”入口、文字镜头编排、直接／局部 AI 修改、保存与恢复。视觉及资产设计先形成可审阅候选。
2. 第二期：补齐项目资产设计与变体、图片和视频提示词编译、稳定引用、局部锁定、版本比较和变更影响。正文改动后定位受影响镜头，不静默覆盖用户手改。
3. 第三期：交付检查、定向语义审阅、不可变版本包、可读文档与 JSON 导出、下游格式适配及联调。待生成图片是正常交接状态，缺失正文来源或失效依赖需先处理。

验收围绕固定题材样本的动作对白覆盖、空间和状态连续性、资产一致性、局部修改量、暂停恢复、版本可追溯与下游准备时间；具体条件由 44 号方案维护。正文质量与现有长篇工作流不得回退，不能把通用 Prompt 导出误写为已通过真实媒体生成验收。

## Downstream Media Production

实际图片、语音、动画与视频生成、选片、模型执行重试、字幕制作、剪辑、成片管理和发布由下游系统承担。本团队可按约定输出素材绑定要求和适配后的文字格式；不建设上述执行链路，也不把厂商参数塞进正式 `MasterScript`。
