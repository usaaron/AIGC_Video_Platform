  ...
  async generate(request: ImageGenerationRequest): Promise<ImageGenerationOutput[]> {
    ...
    for (const view of request.outputs) {
      ...
      const response = references.length
        ? await this.edit(model, prompt, request.aspectRatio, quality, references, idempotencyKey)
        : await this.create(model, prompt, request.aspectRatio, quality, idempotencyKey)
      outputs.push({
        view,
...
function parseProviderImageResponse(text: string, responseContentType = ''): ProviderImageCandidate | null {
...
function parseProviderImageStream(text: string): ProviderImageCandidate | null {
  ...
  for (const event of parseProviderImageEvents(text)) {
    ...
    if (
      ...
      throw new TokenAdventImageResponseError(errorMessage || 'TokenAdvent 图片流返回失败事件')
    }
...
function parseProviderImageEvents(text: string): Array<{ type: string; payload: unknown }> {
...
function decodeBase64Image(value: string, declaredContentType: string): ResolvedProviderImage {
  ...
  return resolvedImage(Buffer.from(base64, 'base64'), dataUrlContentType || declaredContentType)
}
...
function imageContentType(content: Buffer, declaredContentType: string): string {
  ...
  if (content.length >= 8 && content[0] === 0x89 && content.subarray(1, 4).toString('ascii') === 'PNG') {
    return 'image/png'
  }
...
function looksLikeBase64(value: string): boolean {
  ...
  return normalized.length >= 4 && normalized.length % 4 === 0 && /^[A-Za-z0-9+/]+={0,2}$/.test(normalized)
}

function payloadErrorMessage(value: unknown): string {
  if (!isRecord(value)) return ''
  if (value.error) return stringifyMessage(value.error)
  return typeof value.message === 'string' && !providerImageCandidates(value).length ? value.message : ''
}
