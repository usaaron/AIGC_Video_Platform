...
const configSchema = z
  .object({
    ...
    WECHAT_OPEN_PLATFORM_APP_ID: z.string().default(''),
    WECHAT_OPEN_PLATFORM_APP_SECRET: z.string().default(''),
    WECHAT_QR_REDIRECT_URI: z.union([z.literal(''), z.string().url()]).default(''),
    SMS_PROVIDER: z.enum(['none', 'alicloud']).default('none'),
    ALICLOUD_ACCESS_KEY_ID: z.string().default(''),
    ALICLOUD_ACCESS_KEY_SECRET: z.string().default(''),
    ALICLOUD_SMS_SIGN_NAME: z.string().default(''),
    ALICLOUD_SMS_TEMPLATE_CODE: z.string().default(''),
    ...
    VIDEO_STATUS_TIMEOUT_MS: z.coerce.number().int().min(2_000).max(30_000).default(10_000),
    VIDEO_POLL_CONCURRENCY: z.coerce.number().int().min(1).max(32).default(6),
    VIDEO_PROCESSING_STALL_TIMEOUT_MS: z.coerce.number().int().min(60_000).max(1_800_000).default(360_000),
    VIDEO_PROCESSING_TIMEOUT_MS: z.coerce.number().int().min(60_000).max(7_200_000).default(1_800_000),
