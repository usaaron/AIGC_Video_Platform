...
6. 覆盖每集推动主线或多次出现的有名角色，人物名称固定为“人物名-标准版本”或“人物名-具体服装版本”，sourceFacts 必须写基础人物、版本和服装（材质、剪裁、配色、鞋履、固定配饰）。同一造型跨集复用完整名称，只在明确换装时新增版本，不按集数、动作或情绪拆出新人物；网剧不返回 costume；场景只保留复用率高或制作成本高的地点，建议 1 到 4 个；道具只保留重要且会多次出现或承载剧情转折的物件，建议 1 到 5 个；服装只保留角色一致性需要的核心服装，建议 1 到 4 个；广告或片尾出现的品牌、Logo、产品标识建议 1 到 2 个。
