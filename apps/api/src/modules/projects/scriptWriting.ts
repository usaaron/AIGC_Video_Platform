...
- 正文前固定输出“资产：”，资产清单结束后输出“正文：”。每个稳定实体单独一行，名称后使用“｜”补充可复用设定：人物写“人物：角色名-标准版本｜基础人物：角色名｜版本：标准版本｜性别：按原稿｜年龄段：按原稿｜年龄：按原稿｜身份：按原稿｜体型：具体体型｜脸型：具体脸型｜发型：具体发色与长度｜肤色：具体肤色｜基础造型：符合身份的造型”；场景写“场景：名称｜空间：室外｜时代：现代｜场景用途：通行巷道｜固定布局：狭窄直巷｜入口出口：南口、北口｜固定陈设：右侧雨棚、巷尾卷帘门｜材质：旧砖墙和湿水泥地｜基础色彩：冷灰绿”；物品写“物品：名称｜分类：容器｜尺度：手掌大小｜形状：长方体｜材质：金属｜颜色：暗灰｜固定结构：暗扣盒盖｜基础状态：旧但完整｜归属：原稿所属角色名｜准确文字：无”；服装写“服装：名称｜归属：原稿所属角色名｜时代：现代｜季节：秋冬｜剪裁：短款｜层次：单层外套｜颜色：深灰｜材质：防水布｜配件：无”；品牌写“品牌：名称｜准确文字：名称｜图形结构：文字标｜字体方向：横排｜标准色：黑白｜版式背景：透明背景”。没有的类别不要输出。
...
- 对白必须按真实发生时机穿插在动作之间，格式使用“角色名：“对白内容。””；背景说明使用“画外音：“背景事实。””，人物心理使用“内心独白：“角色心理。””；音效使用“[音效]实际发生的声音。”，全部内容按本故事填写。18 到 25 秒场次通常拆成 2 个导演镜头，因此每场准备约 8 句短对白，前后半段各约 4 句；其他时长按每个导演镜头约 4 句换算，每个镜头允许 3 到 5 句。单句尽量 5 到 14 个中文字符；每句用于提问、回答、施压、揭示信息或改变决定，不寒暄、不复述画面中已经看得见的动作。
...
export function webSeriesDialoguePlan(sceneSeconds = 20): {
  shotCount: number
  minimum: number
  target: number
  maximum: number
...
function webSeriesDialogueDensityIssues(script: string): string[] {
...
function parseSceneDurationSeconds(value: string | undefined): number {
  ...
  return secondMatch?.[1] ? Math.max(1, Math.round(Number(secondMatch[1]))) : 20
}
...
export function segmentScriptIssues(segment: string, mode: ScriptContentMode = 'short-video'): string[] {
  ...
  if (mode === 'web-series') issues.push(...webSeriesDialogueDensityIssues(segment))
  return issues
}
...
export function quickScriptIssues(
  ...
  if (mode === 'web-series') issues.push(...webSeriesDialogueDensityIssues(script))
  return issues
}
...
- 角色姓名、身份和关系以用户素材、已有剧本与已确认资产为准；续写沿用已有角色。格式示例仅说明结构，绝不能把示例姓名、职业、服装或情节当作故事事实。“原稿角色名”等占位词必须换成故事中的实际名称。仅在原创输入未命名且确实需要人物时，依据本故事背景自行命名；不要套用固定主角或把无关示例带入正文。
...
- 网剧人物每个稳定造型单独一行，仍归属于同一基础人物，格式示例：“人物：原稿角色名-标准版本｜基础人物：原稿角色名｜版本：标准版本｜身份：原稿身份｜外形：符合原稿的具体外形｜服装：符合原稿的材质、剪裁与配色”。名称固定为“人物名-具体版本”，未换装也标记标准版本；正文中人物首次出场或换装时用括号标明对应版本，对白仍使用人物本名。有明确换装才增加“原稿角色名-具体服装版本”等行；不要为每一集自动创造新造型。网剧不单列服装资产；广告、短片需要独立服装设计时可单列。
- 续集沿用已有版本的完整名称、材质、剪裁、颜色、鞋履和固定配饰，相同造型不要改名重建。同一基础人物的脸型、年龄、五官、体态保持一致；服饰版本描述只覆盖服装与相关固定配饰，不因换装生成新面孔。临时动作、情绪、伤势、天气变化不是新的资产版本。
