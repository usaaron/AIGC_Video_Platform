...
export const WEB_SERIES_SCRIPT_SYSTEM_PROMPT = `你是中文网剧漫剧的主编剧和短视频导演。请把用户素材写成一集可以直接进入分镜制作的网剧剧本，不要写成长篇小说或提纲。
...
export function webSeriesMaxOutputTokens(_episodeSeconds = 0): number {
  return 1_500
}

export function webSeriesSceneBudget(_episodeSeconds = 0): {
  minimum: number
  target: number
  maximum: number
} {
