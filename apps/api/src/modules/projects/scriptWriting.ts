...
- 在正文场次之前先输出可读的稳定资产清单，固定从“资产：”开始，以“正文：”结束；每个实体单独一行，字段使用“｜”分隔。格式示例：资产： / 人物：林晚｜性别：女｜年龄：28岁｜身份：急诊护士｜故事作用：救下程野；场景：废弃药店｜空间：室内｜时间：黄昏｜天气：阴天｜氛围：破败阴森｜故事作用：暂时防守；物品：铁棍｜材质：金属｜状态：磨损｜持有人：林晚｜故事作用：顶住卷帘门；服装：林晚灰色防风服｜归属：林晚｜颜色：灰色｜款式：短款防风外套｜故事作用：保持人物连续；品牌：品牌名｜文字：品牌名｜用途：片尾落版｜故事作用：品牌识别。这里只列剧本已经明确或本次创作确定的稳定资产，没有的类别不要输出。
- 资产清单中的名称必须是稳定、可复用的实体名称；性别、年龄、身份、故事作用、空间、氛围、材质、状态、位置和用途等事实写进对应字段。不要把动作、表情、伤势、镜头、临时天气或完整句子写进资产名称。资产清单中的事实必须在下方正文中保持一致。
...
const FAST_WEB_SERIES_SCENE_RULES = `网剧场次必须同时承担编剧信息、资产引用和导演执行信息，但镜头边界按叙事与摄影目的划分，严禁按单个动作拆镜。
- 在正文场次之前先输出可读的稳定资产清单，固定格式为“资产：”开头、“正文：”结束。每个实体单独一行并使用“｜”分隔事实：人物写“人物：名称｜性别：男/女｜年龄：具体岁数或年龄段｜身份：稳定身份｜外形：稳定外观｜故事作用：本集作用”；场景写“场景：名称｜空间：室内/室外｜时间：时段｜天气：可见天气｜氛围：具体氛围｜故事作用：本集作用”；物品写“物品：名称｜材质：材质｜状态：稳定状态｜持有人：角色｜故事作用：剧情用途”；服装写“服装：名称｜归属：角色｜颜色：配色｜款式：稳定款式｜故事作用：连续性用途”。没有的类别不要输出。
- 资产清单只写下方正文真实使用的稳定资产和可证实信息，禁止使用“不限”“未指定”“青年”等无依据默认值；信息不足时根据用户给出的故事和本集正文作一次合理且一致的创作决定。不得把伤势、血迹、污渍、表情、动作或镜头角度当成资产。
- 每行严格按此顺序使用：场次：｜时长：｜剧情：｜目标：｜阻力：｜变化：｜场景：｜角色：｜服装：｜关键物件：｜入场状态：｜动作：｜对白：｜声音：｜镜头1：｜镜头2：｜出场状态：｜衔接：。常规场次写 18 到 25 秒，约 20 秒场次必须写 2 个约 10 秒的导演镜头；确有第三个独立叙事任务时才增加镜头3。
- 场景必须写稳定地点名称、内外空间、时间、可见天气、入口出口、前中后景布局、危险来源与数量区间、可复用陈设和主光来源；“破败阴森”等气氛必须落实为掉漆招牌、碎玻璃、积灰货架、忽明忽暗灯管、风声或撞击声等可见可听证据。
- 角色只使用稳定角色名；服装只写可跨镜复用的服装资产名与基础款式；关键物件只写需要保持一致的物件名称、初始位置和持有人。伤势、血迹、污渍、表情、姿态、天气、动作和摄影选择属于本场临时状态，必须放进入场状态、动作、镜头或出场状态，不能另起人物、服装或物件资产。
- 动作写本场一个连续核心事件，不使用“动作1/动作2”作为镜头边界。必须明确人物在空间中的起点、朝向、移动路线、左右手、与门窗或物件的内外关系、如何观察或验证未知信息，以及动作结束时各人物和物件的位置；角色不能无缘无故知道药品、出口、敌人或线索在哪里。
- 每场对白写 4 到 6 句，格式为“[对白]角色：内容”“[画外音]内容”或“[内心独白]角色：内容”，单句尽量 5 到 14 个中文字符。每句话都要揭示信息、施加压力、回应问题或改变决定；优先短促的一问一答、一压一顶，禁止寒暄、同义重复和用长台词解释可见动作。声音字段另写环境声、动作声和必要音乐，不混入资产名称。
- 镜头1、镜头2必须各自使用同一嵌套格式：“时长：10秒；任务：建立空间与威胁；景别：俯拍大全景；机位：具体位置；运镜：具体运动与落点；内容：多个相关动作组成的连续段落；对白：分配给本镜的对白组；表演：说话者和听者反应；声音：环境声与动作声；光影：光源方向与变化；首帧：精确人物位置和物件状态；尾帧：精确人物位置和物件状态；资产引用：仅列角色、场景、服装和关键物件的稳定名称”。
- 镜头任务按导演目的划分：建立环境或威胁优先俯拍、高机位或大全景；奔跑、撤退和对抗优先人物视线高度的中景跟拍；对白优先中近景、近景或过肩关系构图；情绪变化使用缓慢推近或拉远；证据和关键物件使用特写。一个镜头允许包含多个相互连续的动作，不要为了每次抬手、转头或说一句话另拆镜头。
- 镜头1尾帧必须能直接成为镜头2首帧；衔接必须把人物位置、运动方向、视线、服装、伤势、物件持有状态、门窗状态和光线交给下一场。项目上下文已有资产时必须逐字复用其名称，不得为同一角色的受伤、脏污或情绪变化重复创建资产。
- 每场保持约 450 到 750 个中文字符的有效信息密度，不用形容词或重复动作凑字数。`

const ADVERTISEMENT_PRODUCTION_RULES = `每个广告段落是一条可直接交给分镜师和视频模型的制作记录，不要只写口号或抽象氛围。
- 在广告段落之前先输出可读的稳定资产清单，固定从“资产：”开始，以“正文：”结束；每个实体单独一行，字段使用“｜”分隔。人物写性别、年龄、身份和故事作用，场景写空间、时间、氛围和故事作用，产品/物品写材质、状态、位置和故事作用，品牌写准确文字、用途和故事作用；没有的类别不要输出。
- 资产名称必须是稳定、可复用的实体名称，资产清单中的事实必须在下方广告正文中保持一致；不要把动作、镜头、临时状态或完整句子写进资产名称。
...
2. 资产清单结束后，每个场次单独占一行，从“场次：S01”正文开始；不要输出创作说明、人物小传、全剧摘要、Markdown、表格或 JSON。
3. 每场必须完成“明确目标→实际阻力→因果行动→可见变化→场尾结果”，并给出足以消除生成歧义的空间、方向、数量、左右手、移动路线、观察过程、起止状态和关键物件状态。
...
6. 前场快速建立处境，中场增加阻力并迫使人物选择，末场形成明显变化和下一集钩子；最后场仍使用连续 S 编号，把未解决的钩子写进变化、出场状态和衔接，不要把场次名改成“剧情钩子”。
7. 每场写 4 到 6 句短而有意义的对白、画外音或内心独白，并分配到两个导演镜头；画外音只补充画面无法表达的信息。每场另写环境声、动作声和必要音乐。
...
2. 原稿已经按场次组织时，场次数量、编号和顺序必须保持；每场补齐 18 到 25 秒的可执行信息，并按导演目的规划约 2 个连续镜头，不按动作数量拆镜。
3. 每行严格使用网剧完整字段；缺失的空间、位置、方向、左右手、信息获得过程、对白反应、声音和首尾帧可根据上下文合理补齐，但不能新增改变剧情的角色、道具、能力或空间规则。
4. 每场都要有目标、阻力、变化和结果，动作明确到人物起点、路线、视线、表情、手部和关键物件状态；不要用“沿用上一场”代替具体状态。
...
2. 每场补齐网剧完整字段，常规 18 到 25 秒场次规划约 2 个约 10 秒镜头。镜头按建立环境、推进运动、完成对话、强调证据或落下情绪等导演目的拆分，不按动作数量拆分。
3. 每场保留或补齐 4 到 6 句短对白、画外音或内心独白，并合理分配到镜头1和镜头2；说话者发声时要写听者的视线、停顿和表情反应。
4. 镜头字段必须写清任务、时长、景别、机位、运镜、连续内容、对白组、表演、声音、光影、首帧、尾帧和稳定资产引用；跑动用中景跟拍，对白用中近景、近景或过肩，环境威胁用高位远景，证据用特写，情绪用缓慢推拉。
5. 镜头1尾帧必须直接承接镜头2首帧；场次衔接继续交付人物位置、动作方向、视线、稳定服装、临时伤势、物件和光线状态。
...
3. 每场严格使用网剧完整字段，写清目标、阻力、变化、空间关系、信息获得过程、左右手、移动路线、入场与出场状态，以及镜头1和镜头2的首尾帧连续关系。
4. 每场必须有 4 到 6 句[对白]、[画外音]或[内心独白]，单句尽量 5 到 14 个中文字符，台词必须推进冲突或改变决定；每场另写环境声、动作声与必要音乐。
5. 本段末尾保留高波动钩子：悬念、受辱后反击前一秒、身份或实力即将揭示、关键物件启动或敌人误判；最后场仍使用连续 S 编号，把钩子写进变化、出场状态和衔接，不要直接解决。
