...
export const complianceRiskPolicyProfiles: ComplianceRiskPolicyProfile[] = [
  {
    id: 'film-production',
    label: '影视创作语境',
    reason: '项目或提示词处于影视剧本、分镜、镜头、道具或特效创作语境，低/中风险描述先降噪，明确高危仍保留审查。',
    projectContentTypes: ['short-drama', 'animation'],
    contextTerms: ['影视剧本', '短剧剧本', '剧本', '分镜', '镜头', '场景', '角色', '道具', '特效', '拍摄', '剪辑'],
    categoryThresholds: {
  ...
  },
  {
    ...
    contextTerms: ['历史纪录片', '纪录片', '史料', '历史研究', '历史教材', '博物馆', '二战', '反法西斯', '中立讲解', '中立报道'],
    categoryThresholds: {
  ...
  },
  {
    ...
    contextTerms: ['医学科普', '医学教学', '临床教学', '急救科普', '手术教学', '解剖学', '病理', '医学院', '康复训练', '安全教育'],
    categoryThresholds: {
      graphic_violence: 'high',
      self_harm: 'critical',
    },
...
export const complianceRiskRules: ComplianceRiskRule[] = [
  {
    ...
    groups: [
      ...
      },
      {
        ...
        requiresAny: ['制作', '教程', '配方', '安装', '引爆', '袭击', '杀伤', '隐藏', '投放', '机场', '人群', '车站', '学校'],
    ...
    groups: [
      {
        ...
        terms: ['肢解', '虐杀', '斩首', '内脏', '残肢', '酷刑', '爆头', '血肉模糊', '断肢', 'gore', 'beheading'],
      },
      {
        severity: 'medium',
    ...
    groups: [
      {
        ...
        excludeNear: ['预防', '劝阻', '心理健康', '求助', '公益', '热线', '治疗', '康复', '反自杀', '危机干预'],
      },
      {
        severity: 'critical',
