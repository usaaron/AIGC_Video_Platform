import { listConditions, matchesStoreFilter } from './libraryFilters.js'
import {
  computeStats,
  duplicateGroups,
  groupVersionsByItem,
...
export class AssetLibraryRepository {
  ...
  async syncGenerated(principal: Principal): Promise<void> {
    const records = await automaticCatalogItems(this.database, this.store, principal)
    await this.saveAutomatic(records)
  }

  async saveAutomatic(records: AssetLibraryItemRecord[]): Promise<void> {
    if (!records.length) return
    if (this.database) {
      await this.database.transaction(async (client) => {
        for (const item of records) {
          if (await insertItemFromStore(client, item))
            await insertVersionFromStore(client, versionFromItem(item))
        }
      })
    } else {
      await this.requireStore().mutateLibraryRuntimeCacheAsync((state) => {
        for (const item of records) {
          if (state.assetLibraryItems.some((existing) => existing.id === item.id)) continue
          state.assetLibraryItems.push(item)
          state.assetLibraryItemVersions.push(versionFromItem(item))
        }
      })
    }
  }
