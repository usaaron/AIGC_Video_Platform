...
export class AssetLibraryRepository {
  ...
  async dedupe(principal: Principal): Promise<{ groups: AssetLibraryDuplicateRecordGroup[]; updatedItems: number }> {
    const [items] = await this.readOwnedLibrary(principal)
