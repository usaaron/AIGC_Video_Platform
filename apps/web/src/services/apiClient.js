...
export const api = {
  ...
  planAgent: (input) => request('/agent/plan', json('POST', input)),
  agentRuns: () => request('/agent/runs'),
  agentRun: (runId) => request(`/agent/runs/${runId}`),
  confirmAgentRun: (runId, clientRequestId = crypto.randomUUID()) =>
    request(`/agent/runs/${runId}/confirm`, json('POST', { clientRequestId })),
  pauseAgentRun: (runId) => request(`/agent/runs/${runId}/pause`, { method: 'POST' }),
  ...
  deleteShot: (projectId, shotId) => request(`/projects/${projectId}/shots/${shotId}`, { method: 'DELETE' }),
