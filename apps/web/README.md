...
分镜页默认使用“按场次智能生成”，每个剧本场次对应一个视频镜头；网剧可按当前集独立生成、出片和下载，不会覆盖其他集。同一场景内的连续镜头使用上一镜真实尾帧，换场和换集使用独立首帧并只继承文字剧情状态。“高级：动作级细拆”只供用户主动细拆明确动作。网剧剧本页按“单集生成”工作，每次固定生成 1 集；长篇内容进入“长剧本生成”分组。已完成视频直接在分镜卡片缩略区播放，版本历史仅用于查看和恢复当前版/上一版。
