    ...
    <div className="admin-app">
      <header className="admin-topbar">
        ...
        <div className="topbar-actions">
          ...
          <button className="icon-button" type="button" {...tooltipProps('退出当前后台账号')} onClick={logout}>
            <LogOut size={18} />
          </button>
        </div>
      </header>
...
function CompliancePromptDetailDrawer({
  ...
  return (
    <div className="drawer-backdrop" onClick={onClose}>
      <aside className="side-drawer" role="dialog" aria-modal="true" aria-labelledby={titleId} onClick={(event) => event.stopPropagation()}>
      ...
      <aside className="side-drawer" role="dialog" aria-modal="true" aria-labelledby={titleId} onClick={(event) => event.stopPropagation()}>
        ...
        <div className="drawer-actions">
          ...
          <button
            className="row-button danger"
            type="button"
            disabled={!canEditUser || deleted || user.passwordResetRequired || busy === `force-password:${user.id}`}
            onClick={() => onForcePasswordReset(user)}
          >
            {busy === `force-password:${user.id}` ? (
              <LoaderCircle size={14} className="spin" />
...
function CreateOrganizationUserModal({
  ...
  const selectScope = (scope) => {
    if (scope === 'organization') {
      const organization = organizationOptions.find((item) => item.id === form.organizationId) ?? organizationOptions[0]
      const roles = organization ? assignableRoleOptions(session, organization).filter(roleRequiresOrganization) : []
      onChange({
        ...form,
        scope,
        organizationId: organization?.id ?? '',
